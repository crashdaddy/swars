...
class Films extends Component {
    constructor(props) {
        super(props)
        this.state = {
            nextPage: "https://swapi.dev/api/films/",
            starships: []
        }
    }
                        ...
                        {starshipList.map((starship, idx) =>{
                          ...
                          return(  <Link to={{"pathname": `/film/${starshipID}`}} >
                            <Paper key={idx} elevation={3} style={shadowStyle}>{starship.name}
                                <img src={`${starship.url.replace('swapi.dev/api', 'swarsapi.herokuapp.com/swimg')}1.jpg`} style={{ width: '100%', height: '100%', position: 'absolute', bottom: '2px', left: '0px' }} />
                            </Paper>
                            </Link>)
                    })}
                    </div>
